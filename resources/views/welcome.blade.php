...
<body class="antialiased bg-[#121417]">
    ...
    <main>
        <x-public-navbar />
        <div class="max-w-6xl mx-auto px-4 mt-4 flex justify-end gap-2 text-sm">
            <form action="{{ route('locale.switch', ['locale' => 'fr']) }}" method="POST">
                @csrf
                <button type="submit"
                    class="px-3 py-1 rounded {{ app()->getLocale() === 'fr' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300' }}">FR</button>
            </form>
            <form action="{{ route('locale.switch', ['locale' => 'en']) }}" method="POST">
                @csrf
                <button type="submit"
                    class="px-3 py-1 rounded {{ app()->getLocale() === 'en' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300' }}">EN</button>
            </form>
        </div>
