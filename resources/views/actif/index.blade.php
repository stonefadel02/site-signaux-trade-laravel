 <div class="bg-white w-full rounded-lg p-4">
     <div class="flex items-center   justify-between mb-4">
         <div class="">
             <h6 class="text-lg font-semibold mb-0">Mes Actifs</h6>
             <p class="mb-2 text-sm text-gray-700">Liste des Actifs</p>
         </div>
         <div class="">
             <a href="{{ route('actifs.create') }}"
                 class="inline-flex items-center px-3 py-1 bg-slate-700 text-white rounded-lg shadow hover:bg-blue-700 transition">
                 <i class="ti ti-plus mr-2"></i> Nouveau Actif
             </a>
             ...
             <table class="min-w-full bg-white divide-y divide-gray-200">
                 <thead class="bg-gray-100">
                     <tr>
                         ...
                         <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                             Symbole</th>
                         <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nom
                         </th>
                 ...
                 <tbody class="divide-y divide-gray-100">
                     @forelse($actifs as $actif)
                         <tr class="hover:bg-gray-50">
                             <td class="px-4 py-3 text-sm text-gray-700">{{ $actif->id }}</td>

                             <td class="px-4 py-3 text-sm text-gray-700">{{ $actif->TypeMarche }}</td>
                             <td class="px-4 py-3 text-sm text-gray-700">{{ $actif->Symbole }}</td>
                             <td class="px-4 py-3 text-sm text-gray-700">{{ $actif->Nom }}</td>
                                     ...
                                     <div x-show="open" @click.away="open = false"
                                         ...
                                         <div class="py-1" role="none">
                                             ...
                                             <form action="{{ route('actifs.destroy', $actif) }}" method="POST"
                                                 onsubmit="return confirm('Supprimer cet élément ?')" class="w-full">
                                                 @csrf
                                                 @method('DELETE')
                                                 <button type="submit"
