@extends('layouts.app')
@section('pageTitle', 'Modifier le plan')

@section('content')
    <div class="max-w-7xl mx-auto py-8">
        <div class="bg-white rounded-lg shadow p-8">
            <h1 class="text-xl font-bold mb-6 text-gray-800">Modifier le plan</h1>
            @if ($errors->any())
            ...
            <form action="{{ route('plans.update', $plan) }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="Titre"
                        ...
                        value="{{ old('Titre', $plan->Titre) }}" required>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="DureeEnJours"
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                            value="{{ old('DureeEnJours', $plan->DureeEnJours) }}" required>
                    </div>
                    <div>
                        ...
                        <input type="number" name="NombreDeSignaux"
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                            value="{{ old('NombreDeSignaux', $plan->NombreDeSignaux) }}" required>
                    </div>
                </div>
                <div>
                    ...
                    <textarea name="AutresAvantages" rows="3"
                        class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                        placeholder="Ex: Support VIP&#10;Accès à la communauté&#10;Webinaires mensuels">{{ old('AutresAvantages', is_array($plan->AutresAvantages) ? implode("\n", $plan->AutresAvantages) : implode("\n", json_decode($plan->AutresAvantages) ?? [])) }}</textarea>
                </div>
                <div>
                    <label for="Visibilite" class="block text-sm font-medium text-gray-700 mb-1">Visibilité</label>
                    <select name="Visibilite"
                        class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
                        <option value="PUBLIQUE" {{ old('Visibilite', $plan->Visibilite) == 'PUBLIQUE' ? 'selected' : '' }}>
                            Publique</option>
                        <option value="PRIVEE" {{ old('Visibilite', $plan->Visibilite) == 'PRIVEE' ? 'selected' : '' }}>
                            Privée</option>
                    </select>
                </div>
