...
@php
    $tSteps = __('welcome.how_to_use.steps');
    $steps = [
        [
            'name' => $tSteps[0]['name'],
            'description' => $tSteps[0]['description'],
            'lucide_icon' => 'user-plus',
            'isHighlighted' => false,
        ],
        [
            'name' => $tSteps[1]['name'],
            'description' => $tSteps[1]['description'],
            'image_src' => 'Layer 2.png',
            'isHighlighted' => true,
        ],
        [
            'name' => $tSteps[2]['name'],
            'description' => $tSteps[2]['description'],
            'lucide_icon' => 'credit-card',
            'isHighlighted' => false,
        ],
        [
            'name' => $tSteps[3]['name'],
            'description' => $tSteps[3]['description'],
    ...
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
        <div class="mx-auto max-w-2xl text-center">
            <h2 class="text-3xl font-bold tracking-tight text-white sm:text-[40px]">
                {{ __('welcome.how_to_use.title') }}
            </h2>
            <p class="mt-4 text-lg leading-8 text-[23px] text-gray-300">
                {{ __('welcome.how_to_use.subtitle') }}
            </p>
        </div>

        <div class="relative mt-20">
            <div class="absolute left-0 top-[4.5rem] hidden w-full border-t border-dashed border-[#00AFFF] md:block"
                ...
                @foreach ($steps as $step)
                    <div class="flex h-full flex-col items-center text-center">
                        ...
                        <div class="relative mt-4 h-12 w-full">
                            <div class="absolute left-1/2 top-1/2 hidden h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#00AFFF] md:block"
                                aria-hidden="true"></div>
                        </div>
                        ...
                        <div
                            ...
                            <div class="absolute bottom-full left-1/2 -translate-x-1/2 w-0 h-0 border-l-8 border-l-transparent border-r-8 border-r-transparent {{ $step['isHighlighted'] ? 'border-b-8 border-b-[#00AFFF]' : 'border-b-8 border-b-[#1E2027]' }}"
                                aria-hidden="true"></div>

                            {{-- Affichage conditionnel de l'icône ou de l'image --}}
                            @if (isset($step['image_src']))
                                <img src="{{ asset($step['image_src']) }}" alt="{{ $step['name'] }}"
                                    class="mx-auto my-3 h-20 w-20" aria-hidden="true">
